...
export async function startRelay(options: RelayOptions): Promise<{ close(): Promise<void> }> {
   ...
   * Currently uses exact matching (major.minor), allowing patch-level differences.
   */
  function isVersionCompatible(providerVersion: string, relayVersion: string): boolean {
    const parse = (v: string) => v.split('.').map(n => parseInt(n, 10) || 0);
    const [pmajor, pminor] = parse(providerVersion);
    const [rmajor, rminor] = parse(relayVersion);
  ...
  function handleProviderHello(conn: Connection, msg: WsMessage): void {
    ...
    if (!payload.protocol_version) {
      conn.send({
        type: 'provider_ack',
        payload: { status: 'rejected', reason: 'missing_protocol_version' },
        timestamp: Date.now(),
      });
      return;
    }

    const negotiatedVersion = isVersionCompatible(payload.protocol_version, String(PROTOCOL_VERSION))
      ? String(PROTOCOL_VERSION)
      : payload.protocol_version;

    if (!isVersionCompatible(payload.protocol_version, String(PROTOCOL_VERSION))) {
      conn.send({
        type: 'provider_ack',
        payload: { status: 'rejected', reason: 'incompatible_protocol_version', negotiated_version: negotiatedVersion },
        timestamp: Date.now(),
      });
      return;
    }
    ...
    conn.send({
      type: 'provider_ack',
      payload: { status: 'accepted', negotiated_version: negotiatedVersion },
      timestamp: Date.now(),
    });
