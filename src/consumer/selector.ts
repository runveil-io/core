...
export class ProviderSelector {
  ...
  selectProvider(criteria?: SelectionCriteria): ProviderInfo | null {
    ...
    if (available.length === 0) {
      console.log('No available providers after filtering');
      return null;
    }
    ...
    console.log({ pubkey: top.provider.pubkey, score: top.score }, 'Selected provider');
    return top.provider;
  }
  ...
  markProviderFailed(pubkey: string): void {
    ...
    if (circuit.state === 'half-open') {
      ...
      console.log({ pubkey }, 'Circuit breaker: half-open ΓåÆ open (failure in probe)');
      return;
    }

    if (circuit.failures >= CIRCUIT_FAILURE_THRESHOLD) {
      circuit.state = 'open';
      console.log({ pubkey, failures: circuit.failures }, 'Circuit breaker: closed ΓåÆ open');
    }
  }
  ...
  markProviderSuccess(pubkey: string, latencyMs?: number): void {
    ...
    if (circuit) {
      if (circuit.state === 'half-open') {
        ...
        circuit.failures = 0;
        circuit.halfOpenAttempts = 0;
        console.log({ pubkey }, 'Circuit breaker: half-open ΓåÆ closed (success)');
      }
    ...
    for (const [pubkey, circuit] of this.circuits.entries()) {
      if (circuit.state === 'open') {
        ...
        if (elapsed >= CIRCUIT_OPEN_DURATION_MS) {
          ...
          circuit.halfOpenAttempts = 0;
          resetCount++;
          console.log({ pubkey, elapsedMs: elapsed }, 'Circuit breaker: open ΓåÆ half-open (reset)');
        }
      }
    }

    if (resetCount > 0) {
      console.log({ resetCount }, 'Reset failed providers to half-open');
    }
  }
  ...
  forceResetAllProviders(): void {
    for (const [pubkey, circuit] of this.circuits.entries()) {
      ...
      circuit.halfOpenAttempts = 0;
      this.failedAt.delete(pubkey);
      console.log({ pubkey }, 'Force reset provider circuit');
    }
  }
    ...
    return this.providers.filter((p) => {
      ...
      if (circuit.state === 'open') {
        ...
        if (elapsed >= CIRCUIT_OPEN_DURATION_MS) {
          circuit.state = 'half-open';
          circuit.halfOpenAttempts = 0;
          console.log({ pubkey: p.pubkey }, 'Circuit auto-transitioned open ΓåÆ half-open');
        } else {
          return false;
        }
      }
